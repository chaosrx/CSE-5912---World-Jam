...
public class AIPathing : MonoBehaviour {
	...
	IEnumerator FollowPath () {
		...
		while (followingPath) {
			...
			if (currentNode.Previous != null && currentNode.Previous.Value.type == AIPathNode.NodeType.Wall) {
				print ("Wall Jump");
				control.Jump ();
			}
			...
			if (!stopping) {
				...
				if (currentNode != subTargets.Last) {
					if ((currentNode.Next.Value.type != AIPathNode.NodeType.Wall && nextNodeDistance < nodeDistance) || flatDist <= characterRadius) {
						NodeAdvance ();
					}
					else if (currentNode.Value.type == AIPathNode.NodeType.Wall && this.transform.position.y >= currentNode.Value.transform.position.y) {
						NodeAdvance ();
						if (currentNode.Value.type != AIPathNode.NodeType.Wall) {
							Invoke ("AttemptBurstDash", dashDelay);
						}
					}
					else if (currentNode.Value.type == AIPathNode.NodeType.Wall && flatDist <= 7) {
						print ("To the Wall Jump");
						control.Jump ();
					}

					if (currentNode.Value.type == AIPathNode.NodeType.Wall) {
						Vector2 jumpTargetFlatPos = new Vector2 (currentNode.Value.wallJumpTarget.transform.position.x, currentNode.Value.wallJumpTarget.transform.position.z);
						float jumpTargetFlatDist = Vector2.Distance (aiFlatPos, jumpTargetFlatPos);
						if (jumpTargetFlatDist <= characterRadius) {
				...
				/* else if (ShouldJump (currentNode.Value.transform.position)) {   // Jump check
					control.Jump ();
					Invoke ("AttemptBurstDash", dashDelay);
					print ("SHOULD Jump");
				}*/
	...
	public float jumpHeightDiff = characterRadius*3f;
