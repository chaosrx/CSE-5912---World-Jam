...
public class PlayerControllerMulti : NetworkBehaviour
{
	...
	Color[] colors = { Color.blue, Color.red, Color.green, Color.yellow, new Color(4,1,0), Color.white, Color.magenta };
    ...
    void Start()
    {
        ...
        StartCoroutine("SetColorMulti");

        this.GetComponent<SpeedWarpCharge>().enabled = false;
    ...
    void FixedUpdate()
    {
        ...
        if (!setColor && PlayerID != 0)
        {
			...
			l.color = colors [(PlayerID % colors.Length)];
        }
        **/
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        {
            if (col.gameObject.GetComponent<BurstDashMulti>().IsDashing || col.gameObject.GetComponent<SlideMulti2>().IsSliding)
            {
